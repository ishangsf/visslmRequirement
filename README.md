...
1. 打开“系统配置”，填写 VISSLM 平台地址、用户名、API Token，以及用于用户显示名查询和富文本图片上传网页登录的平台登录密码，然后测试并保存。API Token 用于 REST 接口；平台登录密码用于用户显示名查询和富文本图片上传。配置了非空用户属性 Key 时，采集需要该密码；两类凭据均使用操作系统安全存储加密，界面不回显明文。
...
- API Token、图片上传平台登录密码和在线模型 API Key 使用操作系统安全存储加密；日志和请求预览不会输出 Token、密码或 Cookie 值。
