...
- 正文中的可用图片资源在每次推送时调用 `FileCenterImg/UploadRichImg`；同一条记录内内容相同的图片只上传一次并复用本次返回路径。缺失、未解析或上传失败的图片会跳过，并使用“图片未能上传”占位，文字内容仍调用 `/rest/items` 创建记录。创建成功但缺图时显示“已创建（部分图片未上传）”，图片失败原因单独列出；只有记录创建接口失败才计为记录推送失败，勿因图片警告重复创建记录。
