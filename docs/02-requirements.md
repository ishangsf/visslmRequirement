...
- 系统根据项目需求标题、正文、模块、分类和确定性业务事实自动召回历史需求，并按版本化综合匹配分排序。
- 运行和候选结果不可变；默认读取当前需求快照的最新兼容成功运行。
- 仅精确业务哈希可自动确认“重复”，任何语义候选均不得自动关联资产或修改需求状态。
- 自动化评测不使用人工标签，其结论限于确定性安全、稳定性和一致性，不代表开放域准确率。
