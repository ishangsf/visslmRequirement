...
14. **核心流程**：路由专家 -> 通用助手计划/工具调用、需求分析专家编号定位与语义卡片 -> Dense/FTS5/BM25/结构化字段 RRF 召回 -> 本地 Cross-Encoder 重排 -> AI 初审与独立复核 -> UID/证据/关系程序校验 -> 返回 answer/sources/dataViews；会话由前端保存。
...
22. **已知问题**：需求分析的 AI 复核依赖配置中的模型服务；本地 Cross-Encoder 和 embedding 资源必须完整，任一不可用会失败关闭；真实人工标注集、模型对比和概率校准尚未完成；对话/工具结果无统一可观测 trace；在线数据外发没有统一审批。
