...
`pm_requirement_match_runs` 保存需求快照哈希、归一化/流水线/排序版本、配置哈希、模型版本、状态、降级码和完成时间。`pm_requirement_match_candidates` 以运行和记录唯一，保存阶段分数、最终排名、关系、决策、证据级别、原因码、解释和记录快照哈希。运行与候选采用追加写；完成时在同一事务中写候选并转换状态，失败不得留下部分候选。
