...
- 向量以 BLOB + dimension + model_version 保存，应用侧计算 cosine，不使用 SQLite 向量扩展。
...
| 知识向量 | SQLite `knowledge_vectors` | model_version；模型变化需重建 | 大量向量占用单文件 SQLite，查询全量候选 |
...
`knowledge_documents.file_path` 保留源文件管理路径；原始 PDF 预览由主进程读文件并返回 base64。导出文件由用户在 Electron 对话框中选择路径，不自动上传。
...
主进程退出会中止进程内任务；项目分析在下一次初始化时把遗留 processing 状态改成 failed。同步和知识库任务没有同等级的持久恢复队列。

## 11. 外部系统集成

### 11.1 VISSLM

`VisslmClient` 对平台查询、附件下载、范围预览和 `/alm/rest/items` POST 做封装。响应解析、正文规范化、图片 URL 扫描、过滤器比较、分页递归和请求日志由 `visslm.ts` 完成。平台正式接口 schema、限流、幂等和重试政策未在仓库中声明。
...
- records 搜索使用 FTS5；
- 字段画像和 QuerySpec 结果按数据 revision 缓存；
...
当前性能风险：向量检索在应用侧枚举 SQLite 中的所有候选并计算 cosine；大规模数据、向量和原始 JSON 会使单文件数据库、主进程 event loop 和内存成为瓶颈；没有后台 worker、向量索引、分页向量检索或数据库分片。
...
4. 外部协议的错误重试、限流、幂等和断点续传未形成可配置架构。
