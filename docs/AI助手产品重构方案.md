...
1. 根据编号精确定位基准记录，读取完整清洗原文；查询阶段不生成额外的预生成结构化资产。
2. 在当前 embedding 索引的全部可读记录中执行 Dense 与 SQLite FTS5/BM25 召回，通过 RRF 合并候选。
...
5. 程序严格校验 UID、覆盖范围、JSON schema 和原文证据。解释失败时保留确定性结果并明确标记解释不可用；完整通过校验的解释仅用于本次回答。

模型内部 reasoning 不直接展示。用户看到的是经过复核的结论、证据、主要相同点和差异，以及可打开的真实记录明细。

相似记录查询的固定执行链路为：

`意图归一化 → 编号精确定位 → 完整清洗原文 → Dense/BM25 两路召回 → RRF → Cross-Encoder Top20 → 确定性评分 → Top10 批量解释 → 程序校验 → 答案与 ChatDataView`
...
- 需求匹配要求模型只负责生成经过 UID、schema 和原文证据校验的相似点与差异解释；关系和综合匹配度由程序基于完整清洗原文确定性计算。推理传输参数按已知供应商能力发送，通用 OpenAI 兼容服务只使用标准请求字段，不猜测其支持 `reasoning_effort` 等扩展参数。
- 相似结果使用真实 UID、业务编号、节点类型和名称，并同时展示 Dense/BM25 召回分、Cross-Encoder 重排分、确定性评分明细和 AI 解释状态。
- 增加失败关闭校验：模型新增 UID、遗漏/重复候选、证据不在原文或 schema 不符合时拒绝该批解释；确定性评分结果仍按可见关系和解释状态明确展示。
- 模型状态检查分为两级：首页自动检查只访问模型目录，不触发计费问答；设置页手动“测试模型”额外执行不含业务数据的最小问答，能够识别“模型目录可访问但聊天权限未开通”的半连通状态。
- Agent 请求取消已落地：每轮任务使用独立 `runId`，主进程按窗口隔离运行并把取消信号传递到全部模型供应商的 HTTP 请求；停止后清理运行状态、丢弃未完成证据和产物，并将该轮标记为 `cancelled` / `undone`，不进入后续有效上下文。
- 真实流式回答已落地：Ollama、OpenAI Chat Completions、RawChat Responses 和 Anthropic 均按各自协议增量读取可见答案，并通过 `runId + sequence` 隔离旧任务、重复分片和乱序事件；模型 reasoning、thinking 与工具参数始终留在内部。数据与知识任务先完成查询和证据校验，再交付权威答案；澄清、失败或取消时立即丢弃临时内容，只有最终 `ChatResponse.answer` 会写入会话。
- 模型能力诊断已落地：启动阶段只读取模型目录与 Ollama `/api/show` 元数据，不触发生成；设置页区分“测试连接”和由用户主动执行的“完整能力测试”。能力矩阵分别展示连接、最小问答、JSON Schema、工具调用、上下文长度与思考模式，并区分支持、有限支持、不支持、未验证和检测失败。完整探针使用无业务数据、低 Token、非流式请求，单项能力失败不会被误报成模型离线。
- Agent 工作区就绪诊断已落地：无数据、索引未建立、索引中、索引失败与状态读取失败使用结构化状态区分；最近一次索引任务持久化并在应用重启后恢复。界面提供准备数据、上传文档、刷新状态、查看失败文档和重建索引入口，重建会同时覆盖采集记录与知识文档。
- 执行前计划确认已落地：数据、知识、混合、可视化和需求匹配任务在证据工具执行前发送结构化执行摘要，并按窗口与 `runId` 暂停等待用户确认；摘要包含检索词、读取字段、筛选条件、来源、交付形式、项目、数据类型、指定记录数和结果上限。停止任务会释放确认等待且不访问证据。确认后的实际范围随回答持久化，历史会话恢复时重新应用该会话的数据范围。

## 后续路线图

### P0：保证可用

- ~~增加请求取消：为每次 Agent 任务分配 `runId`，支持停止本地模型请求并清理状态。~~（已完成）
- ~~增加真实流式回答：先展示阶段状态，随后增量展示最终答案；不把内部 reasoning 原文展示给用户。~~（已完成）
- ~~模型配置增加“连接测试 + 最小问答测试”，区分地址、模型、Key、工具调用和结构化输出能力。~~（已完成）
- ~~对本地模型能力做启动探测：JSON Schema、tool calling、上下文长度和思考模式分别标记。~~（已完成）
- ~~增加无数据、索引中、索引失败的明确 CTA 和状态刷新。~~（已完成）

### P1：提升任务成功率

- ~~将计划展示为可折叠的“执行摘要”：检索词、字段、筛选条件和数据范围可在执行前确认。~~（已完成）
- ~~为常用业务字段建立语义词典和人工纠正入口，减少“负责人/来源/状态”等字段映射错误。~~（已完成）
- ~~对查询失败提供安全改写建议，例如缩小时间范围、确认字段名或切换“正文检索”。~~（已完成）
- ~~将回答依据抽象为固定的 EvidenceBlock，统一文档、记录、聚合和查询明细的入口。~~（已完成）
- ~~支持会话级项目范围和数据范围，但每次回答仍显示实际生效范围。~~（已完成）

### P2：从问答走向 Agent

- ~~在用户确认后提供只读之外的操作：创建分析快照、保存筛选视图、生成报告草稿。~~（已完成；同时支持文件交付草稿及 DOCX/XLSX/PPTX/ZIP 导出）
- ~~所有写操作先生成变更预览、影响范围和回滚点，禁止模型直接提交。~~（已完成）
- ~~增加任务运行历史、耗时、工具调用、命中数和失败阶段统计，用于产品迭代。~~（已完成）
- ~~建立离线评测集：总量、业务字段统计、属性问答、空结果、歧义问题、提示注入和错误字段。~~（已完成）

### 路线目标完成记录

截至本轮交付，P0、P1、P2 路线项均已完成。新增事实契约包括：`EvidenceBlock`、字段语义词典、`AssistantArtifactPreview`、受控交付物版本、`AssistantRunHistory` 与离线评测集。所有写操作均由主进程校验预览哈希，用户确认后才持久化；撤销保留版本和证据载荷，不修改数据中心或知识库原始内容。
